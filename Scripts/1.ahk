...
AddFriends(renew := false, getFC := false) {
	...
	Loop {
		...
		if(count = 0) {
			...
			FindImageAndClick(226, 100, 270, 135, , "Add", 38, 460, 500)
			FindImageAndClick(96, 453, 103, 472, , "Applying", 138, 461)
			sleep, 750
			AllAccepted := FindOrLoseImage(174, 200, 248, 210, , "Withdraw", 1)
			FindImageAndClick(120, 500, 155, 530, , "Social", 143, 518, 500)
			if(AllAccepted) {
...
GodPackFound(validity) {
	...
	if(validity = "Valid") {
		...
		LogToDiscord(logMessage, screenShot, false, accountFullPath)
	}
}
