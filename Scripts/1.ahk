...
menuDelete() {
	...
	Sleep,%Delay%
	FindImageAndClick(56, 444, 108, 455, , "Account2", 79, 256, 1000) ;wait for account menu
	Sleep,%Delay%
...
FindGodPack() {
	global winTitle, discordUserId, Delay, username, packs, minStars, scriptName, DeadCheck, deleteMethod, mistakeGP
	gpFound := false
	invalidGP := false
	searchVariation := 5
	confirm := false
...
saveAccount(file := "Valid") {
	...
	Loop {
		...
		adbShell.StdIn.WriteLine("su -c 'cp -f /data/data/jp.pokemon.pokemontcgp/shared_prefs/deviceAccount:.xml /sdcard/deviceAccount.xml'")
		waitadb()
		Sleep, 500
...
adbSwipeUp() {
	global adbShell, adbPath, adbPort
	;initializeAdbShell()
	adbShell.StdIn.WriteLine("input swipe 309 876 309 207 60")
	waitadb()
}
